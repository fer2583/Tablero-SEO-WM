...
export function IndexationPanel() {
  ...
  return <div className="indexation-panel">
    ...
    <Card><CardHeader title="URLs reales del sitemap e inspección" detail={`${data.strategy.priority} ${data.strategy.persistence} ${data.inspection.checkedAt ? `Última comprobación: ${date(data.inspection.checkedAt)}.` : ""}`} />{rows.length ? <DataTable rows={rows} columns={columns} rowKey={(row) => row.url} label="URLs de indexación" /> : <EmptyState title="No data" detail="No se encontraron URLs en el sitemap." />}</Card>
