...
export function IndexationPanel() {
  ...
  const load = async (inspect = false) => { setError(""); if (inspect) setInspecting(true); else setLoading(true); try { const response = await fetch(`/api/indexation${inspect ? "?action=inspect-priority" : "?action=refresh-sitemap"}`, { cache: "no-store" }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || "No se pudo cargar Indexación."); setData(payload); } catch (reason) { setError(reason instanceof Error ? reason.message : "Error de conexión"); } finally { setLoading(false); setInspecting(false); } };
  ...
  return <div className="indexation-panel">
    <div className="audit-toolbar"><div><strong>{data.siteUrl}</strong><small>Propiedad GSC: {data.gscSiteUrl} · Actualizado: {date(data.generatedAt)}</small></div><div className="audit-actions"><button className="outline-button small" onClick={() => void load()} disabled={loading}><RefreshCw size={14} /> Actualizar sitemap</button><button className="primary-button" onClick={() => void load(true)} disabled={inspecting}><RefreshCw className={inspecting ? "spin" : ""} size={14} /> Inspeccionar prioritarias</button></div></div>
