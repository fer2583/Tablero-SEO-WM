...
function DashboardContent({ children }: { children: React.ReactNode }) {
  ...
  const filters: IntegrationFilters = { days: ([7, 28, 30, 60, 90].includes(rawDays) ? rawDays : 30) as IntegrationFilters["days"], language: (searchParams.get("language") ?? "all") as IntegrationFilters["language"], country: searchParams.get("country") ?? "all", device: (searchParams.get("device") ?? "all") as IntegrationFilters["device"], page: searchParams.get("page") ?? undefined, query: searchParams.get("query") ?? undefined };
