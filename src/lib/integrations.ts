...
export type AnalyticsData = { period: { start: string; end: string; previousStart: string; previousEnd: string }; metrics: { users: number; newUsers: number; sessions: number; organicSessions: number; aiSessions: number; engagedSessions: number; engagementRate: number; engagementDuration: number; eventCount: number; conversions: number }; previous: Omit<AnalyticsData["metrics"], "organicSessions"> & { organicSessions: number }; landingPages: AnalyticsRow[]; sources: AnalyticsRow[]; aiSources: AnalyticsRow[]; aiLandingPages: AnalyticsRow[]; countries: AnalyticsRow[]; devices: AnalyticsRow[]; dailySessions: number[]; aiStatus: AiTrafficStatus; warnings: string[] };

// GA4 has no universal native AI-referral metric. Keep this list explicit and extensible.
...
export async function fetchAnalytics(filters: IntegrationFilters): Promise<AnalyticsData> {
  ...
  const organicValues = values(organicTotals, 1); const oldOrganicValues = values(oldOrganicTotals, 1); const aiRows = rows(aiSources); const aiSessions = aiRows.reduce((sum, row) => sum + row.sessions, 0);
  const metrics = { users: current[0], newUsers: current[1], sessions: current[2], organicSessions: organicValues[0], aiSessions, engagedSessions: current[3], engagementRate: current[4], engagementDuration: optionalValues[0], eventCount: optionalValues[1], conversions: current[5] };
  const old = { users: previous[0], newUsers: previous[1], sessions: previous[2], organicSessions: oldOrganicValues[0], aiSessions: 0, engagedSessions: previous[3], engagementRate: previous[4], engagementDuration: oldOptionalValues[0], eventCount: oldOptionalValues[1], conversions: previous[5] };
  return { period, metrics, previous: old, landingPages: rows(pages), sources: rows(sources), aiSources: aiRows, aiLandingPages: rows(aiLandingPages), countries: rows(countries), devices: rows(devices), dailySessions: (daily?.data.rows ?? []).map((row) => number(row.metricValues?.[0]?.value)), aiStatus: aiSources ? "available" : "unavailable", warnings };
}
