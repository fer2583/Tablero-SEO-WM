...
export default function SectionPage() { const { section } = useParams<{ section: string }>(); if (section === "indexacion") notFound(); const [label, title, description] = titles[section] ?? titles.configuracion; return <DashboardShell>{section === "analytics" ? <AnalyticsLive /> : section === "search-console" ? <SearchConsoleLive /> : section === "auditoria-tecnica" ? <AuditPanel /> : <><SectionHeading eyebrow={label} title={title} description={description} />{section === "keywords" ? <><FilterBar /><KeywordsLive /></> : section === "contenido" ? <ContentPanel /> : section === "configuracion" ? <ConfigurationPanel /> : <NotConnected section={label} />}</>}</DashboardShell>; }
